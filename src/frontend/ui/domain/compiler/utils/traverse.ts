...
export function getRootNode<T extends TraversableNode>(node: T): T {
  while (node.parent) {
...
export function getSiblingNodes<
  T extends TraversableNode & { parent?: T | null },
>(node: T): T[] {
  if (!node.parent) {
    return []; // 루트 노드는 형제가 없음
  }

  return node.parent.children.filter(
    (child): child is T => child !== undefined && child !== node
  );
}

export function getSiblingPosition(node: SuperTreeNode): {
  hasLeft: boolean;
  hasRight: boolean;
} {
  if (!node.parent) {
  ...
  // undefined가 아닌 유효한 자식들만 필터링
  const validChildren = node.parent.children.filter(
    (child): child is SuperTreeNode => child !== undefined
  );

  const index = validChildren.findIndex((child) => child === node);

  return {
    hasLeft: index > 0,
    hasRight: index < validChildren.length - 1,
  };
}
