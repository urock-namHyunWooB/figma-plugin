...
export interface ImageComparisonResult {
  diffPixels: number;      // 다른 픽셀 수
  totalPixels: number;     // 전체 픽셀 수
  diffPercentage: number;  // 차이 비율 (0-100)
  isMatch: boolean;        // 매칭 여부 (threshold 기준)
  diffImageData?: ImageData;  // 차이 시각화 이미지
}
