import type { ComponentAST, ElementASTNode, IPrettifier } from "../../types";
import SemanticConverter from "./prettifier-converters/SemanticConverter";
import PrettifierStyleConverter from "../style/prettifier-converters/StyleConverter";
...
export class Prettifier implements IPrettifier {
  ...
  private prettifyNode(node: ElementASTNode): ElementASTNode | null {
    ...
    return new SemanticConverter().convert(
      new PrettifierStyleConverter().convert(updatedNode),
    );
