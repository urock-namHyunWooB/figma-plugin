...
describe("Component Runtime 렌더링 테스트", () => {
  describe("기본 렌더링", () => {
    test.skip("생성된 버튼 컴포넌트가 렌더링됨 (styles 스코프 이슈)", async () => {
      const spec = {
  ...
  describe("Props 전달 및 렌더링", () => {
    ...
    test.skip("variant props가 유니온 타입으로 작동 (styles 스코프 이슈)", async () => {
      const spec = {
        metadata: { name: "Button", rootElement: "button" },
        propsDefinition: [
          {
  ...
  describe("스타일 렌더링", () => {
    test("배경색이 적용됨", async () => {
      ...
      expect(box?.getAttribute("style")).toContain("background");
      expect(box?.getAttribute("style")).toContain("rgb(255, 0, 0)");
    });
    ...
    test("테두리가 적용됨", async () => {
      ...
      expect(box).toBeTruthy();
      expect(box?.getAttribute("style")).toContain("border");
      expect(box?.getAttribute("style")).toContain("2px solid rgb(0, 0, 0)");
    });
    ...
    test("ELLIPSE는 원형으로 렌더링", async () => {
      ...
      // React inline style은 camelCase이지만 getAttribute는 kebab-case로 변환됨
      expect(circle?.getAttribute("style")).toContain("border-radius: 50%");
      expect(circle?.getAttribute("style")).toContain("rgb(0, 0, 255)");
    });
  });
