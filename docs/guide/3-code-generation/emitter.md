...
**책임 분리**: native HTML prop 충돌 rename은 Layer 3(ReactEmitter)에서만 수행한다. Layer 2의 StyleProcessor는 `normalizePropName()`으로 prop 이름 정규화만 하고 rename 로직(`isNativePropConflict`)은 포함하지 않는다.
...
- 슬롯 바인딩: `{slotProp && (<div css={styles}>{slotProp}</div>)}`
  - **예외**: `bindings.attrs`에 placeholder가 설정된 노드는 slot으로 렌더링하지 않고 `<input>` 태그로 렌더링
- 배열 슬롯: `{items.map((item, index) => <Item key={index} {...item} />)}`
- `generateInputElement()`: search-input 및 placeholder semanticType 노드를 `<input>` 태그로 렌더링 (기존 `generateSearchInputNode`에서 이름 변경, search-input과 placeholder 공용)
