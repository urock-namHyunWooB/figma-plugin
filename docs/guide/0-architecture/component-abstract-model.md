...
1~5번이 끝나면 UITree가 완성된다. 이후 CodeEmitter가 이 UITree를 받아서 특정 프레임워크(React)와 스타일 전략(Emotion/Tailwind)에 맞게 코드로 출력한다. UITree가 플랫폼 독립적이기 때문에, CodeEmitter만 바꾸면 다른 프레임워크로도 출력할 수 있는 구조다.
