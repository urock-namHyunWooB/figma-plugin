...
4-way 비교가 실패할 수 있습니다. 이를 4-way 비교 **이전에** 보정량을 계산하여
offset에 적용하고, 보정된 위치로 4-way 비교를 수행합니다.
...
기존 설계(4-way 실패 후 fallback)와의 차이:
- 이전: 4-way 실패 → fallback에서 절대좌표 ±10px 비교
- 현재: 4-way **이전에** shift 적용 → 4-way 상대 비교(±0.1)로 판정
