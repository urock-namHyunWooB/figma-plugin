...
### 6.5 문서 (docs/ + CLAUDE.md)

**원칙**: Historical 문서(작성 시점 상태를 기록하는 것이 목적)는 건드리지 않고, current 문서(현재 코드베이스 상태를 설명하는 것이 목적)만 업데이트한다.

**Historical — 업데이트 안 함**:
- `docs/superpowers/plans/*.md` — 과거 작업 기록
- `docs/superpowers/specs/2026-04-08-variant-merger-engine-design.md` — 당시 설계 시점의 디자인 결정 기록
- `docs/superpowers/specs/2026-04-09-*.md` — 이전 handoff/시도 기록

**Current — 업데이트 함**:
- `docs/superpowers/specs/2026-04-10-hungarian-observation-tool-design.md` (Spec B) — 후속 작업이 새 경로를 전제로 하므로 일관성 유지 필요 (브레인스토밍 중 이미 업데이트됨)
- `CLAUDE.md` — 프로젝트 현재 상태 설명 문서. "Cross-Depth Squash (UpdateSquashByIou)" 같은 언급은 현재 코드의 클래스명을 반영해야 함 → `VariantSquasher`로 치환 (2개 라인)
...
- current docs: 2개 (Spec B는 브레인스토밍 중 이미 업데이트됨, CLAUDE.md 클래스명 2건)
